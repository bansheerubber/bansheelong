...
async fn service(request: Request<Body>, io: Arc<Mutex<IO>>) -> Result<Response<Body>, Infallible> {
	...
	match (request.method(), request.uri().path()) {
		(&Method::GET, "/get-todos/") | (&Method::GET, "/get-todos") => {
			let mut guard = io.lock().expect("Already held");
			let result = guard.read_database();
		...
		(&Method::GET, "/add-todo/") | (&Method::GET, "/add-todo") => {
			...
			let mut guard = io.lock().expect("Already held");
			let result = guard.add_to_database_sync(Item::new(&parameters), Date::new(&parameters));
			if let Err(error) = result {
				eprintln!(" -> Error on request, {:?}", error);
				return Ok(
					Response::builder()
...
pub async fn test(io: Arc<Mutex<IO>>) -> hyper::Result<()> {
